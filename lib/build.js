...
function buildJavaScript(root, name, asset, config) {
  var sourceFiles = _.map(asset.sourcePaths, function(sourcePath) {
    return path.join(root, asset.sourceRoot, sourcePath);
  });
  var destFile = path.join(root, asset.destRoot, asset.destPath);
