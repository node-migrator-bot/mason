...
function FileTree(data, file) {
  var self = this;
  this.doOnChange = _.bind(this.doOnChange, this);
  this.delay = 1000;
  
  this.changedSinceLastDebounce = false;
  this.onChange2 = _.debounce(function() {
    console.log("calling onChange before debounce");
    self.doOnChange.apply(self, arguments);
    self.changedSinceLastDebounce = false;
  }, this.delay, true);
  this.onChange1 = _.debounce(function() {
    if(!self.changedSinceLastDebounce) return;
    self.changedSinceLastDebounce = false;
    console.log("calling onChange after debounce");
    self.doOnChange.apply(self, arguments);
  }, this.delay);
...
FileTree.prototype.watch = function() {
  ...
  function createWatcher() {
    self.watcher = fs.watch(self.file, function() {
      self.changedSinceLastDebounce = true;
      self.onChange1.apply(self, arguments);
      self.onChange2.apply(self, arguments);
    });
  }
};
FileTree.prototype.doOnChange = function(event) {
