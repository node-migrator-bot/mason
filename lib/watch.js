...
FileTree.prototype.watch = function() {
  ...
  if (this.file) {
    if(this.fileExists()) {
      ...
      this.watcher = fs.watch(path.dirname(this.file), function() {
        if(self.fileExists()) {
          self.watcher.close();
          createWatcher();
          self.onChange();
...
FileTree.prototype.onChange = function(event) {
  ...
  console.log('\033[32mMason:\033[0m \033[36mchanged\033[0m\033[90m - %s\033[0m', this.file);
...
module.exports = function watch(root, config) {
  ...
  function rebuild(names) {
    try {
      ...
      console.log('\033[41m ERROR \033[0m building file ' + fileName);
      console.log(e.stack);
    }
  }
};
